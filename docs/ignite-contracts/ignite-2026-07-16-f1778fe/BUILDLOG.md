...
- [x] **Scoped unfreeze: service auth for the tests-platform adapter** (task #107, 2026-07-16; requested by tests-platform at the start of its Phase 2 CAT-adapter work). (1) **OAuth2 client-credentials grant**: `POST /auth/token` (RFC 6749 §4.4, form fields or HTTP Basic) mints the existing short-lived HS256 JWT with `token_use=service` + a `scope` claim; the credential is env-injected (`SERVICE_CLIENT_ID`/`SERVICE_CLIENT_SECRET`, secrets-manager-sourced in production, grant disabled when unset) with a `SECONDARY` slot so two secrets are active during rotation (constant-time compares). A principal row (`svc:<client_id>`, role=`service`, unusable password hash — `/auth/login` rejects it; new alembic `a7b8c9d0e1f2` widens `users_role_check`) is lazily provisioned solely for FK ownership of what the client creates; deactivating it via DELETE /admin/users is the operator kill switch. (2) **Least-privilege enforcement at ONE boundary** (the C1 philosophy applied to auth): a scope→route allowlist checked centrally in `get_current_user` before any endpoint code — service tokens get exactly create+read test-configs, create/drive/read sessions, ingest+read own item banks; new endpoints are service-inaccessible by default; 15 integration tests pin both the allowed surface (incl. a full token→config→session→response→abort drive) and the denials (/admin, /simulations, DELETE verbs, password login as the principal). (3) The **privileged-account TODO** at `api/auth.py:42` was stale — `POST /admin/users` (task #81) already mints role-assigned accounts; docstring now says so. (4) **`/ready` implemented for real**: parallel pings of postgres/redis/mirtcat/neural (reusing the admin system-status helpers), 200 only when all healthy, 503 with per-dependency detail otherwise, unauthenticated by design. OpenAPI snapshot + orval client regenerated; 243 backend tests green.
- [x] **Scoped unfreeze: blueprint-contract refresh — upstream `tcc_target`** (task #106, 2026-07-16; advance drift notice from tests-platform per the contract-pack tripwire). Upstream added an optional `tcc_target` expected-score band (TCC comparability for fixed-form/LOFT assembly — a design-time construct CAT ignores at runtime, same class as the TIF target). Re-vendored via `scripts/refresh_blueprint_contract.sh` (new sha256 `2f3a5675…`), confirmed additive-only by diff review; `binding_warnings()` now surfaces `tcc_target` as CAT-ignored alongside `statistical_target`/`exposure_target`; regression tests pin both the warning and that the vendored contract's own invariants (score ≤ form length) still run at binding time. Live-verified: a `tcc_target`-bearing blueprint saves through `POST /test-configs/` and round-trips intact.
