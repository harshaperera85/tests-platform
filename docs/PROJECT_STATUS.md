...
| **§4 LOFT engine (merge-gate)** | — | Per-session conforming assembly per BP-MODES-1 §4: `LoftStrategy` + `assembly/loft.py` — TIF band as hard acceptance (§4.1), running exposure-rate cap (§4.2, batch fields ignored w/ warning), engines (a) seeded random search / (b) per-session CP-SAT w/ randomized diversity objective (§4.3), conformance records true by construction (§4.4). `POST /loft/sessions` doubles as the §7 verification surface; CI-sized run: 150 sessions 100% conformant, band held, exposure ≤ rate+ε, diverse. |
...
| **Field-study assembly path** | — | Content-only forms from uncalibrated items: imported banks derive a `<bank>-field` pool (editorial live+pilot, parameters never exposed); `FieldPool` type gates the parametric paths; compiler/strategies assemble pure-feasibility; reporting degrades honestly (QA content-only, no TIF, simulate/compare guarded). Closes the calibration bootstrap's missing middle: import → field form → (Sessions) → responses → scoring-r `/calibrate` → write-back. |
